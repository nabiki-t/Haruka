...
// MemoryMediaPropPage.fs : Implement the function to display Memory Buffer Media property page in the main window.
//
...
type MemBufferMediaPropPage(
    ...
    let m_BytesCountTextBox = m_PropPage.FindName( "BytesCountTextBox" ) :?> TextBox
    ...
    do
        ...
        m_Config.SetLocalizedText "MemBufferMediaPropPage" m_PropPage
        ...
        m_UsageExpander.Expanded.AddHandler ( fun _ _ -> UserOpeStat.SetExpanded "MemBufferMediaPropPage" m_UsageExpander.Name true )
        m_UsageExpander.Collapsed.AddHandler ( fun _ _ -> UserOpeStat.SetExpanded "MemBufferMediaPropPage" m_UsageExpander.Name false )
        m_ConfigurationExpander.Expanded.AddHandler ( fun _ _ -> UserOpeStat.SetExpanded "MemBufferMediaPropPage" m_ConfigurationExpander.Name true )
        m_ConfigurationExpander.Collapsed.AddHandler ( fun _ _ -> UserOpeStat.SetExpanded "MemBufferMediaPropPage" m_ConfigurationExpander.Name false )
        ...
        // Set default value
        m_UsageExpander.IsExpanded <- UserOpeStat.GetExpanded "MemBufferMediaPropPage" m_UsageExpander.Name
        m_ConfigurationExpander.IsExpanded <- UserOpeStat.GetExpanded "MemBufferMediaPropPage" m_ConfigurationExpander.Name
    ...
    member private this.OnClick_ApplyButton() : unit = 
        try
            let mn = m_ServerStatus.GetNode m_NodeID :?> ConfNode_MemBufferMedia
            ...
            let bytesCount =
                let r, w = UInt64.TryParse m_BytesCountTextBox.Text
                if not r then
                    let msg = m_Config.MessagesText.GetMessage( "MSG_INVALID_MEMBUFFER_BYTES_COUNT" )
                    raise <| Exception msg
                w
    ...
    member private _.OnTimer() : unit = 
        ...
        m_MainWindow.ProcCtrlQuery false ( fun () -> task {
            ...
            let tdActived = Seq.exists ( (=) tdn.TargetDeviceID ) activeTDID

            // Get the ancestor target group status
            let loadedTG = m_MainWindow.GetLoadedTGIDs()

            let! luStat, tgLoaded = task {
                if tdActived then
                    let loaded =
                        match loadedTG.TryGetValue tdn.TargetDeviceID with
                        | true, v ->
                            Seq.exists ( (=) tgn.TargetGroupID ) v
                        | false, _ ->
                            false
                    if loaded then
                        let! ws = m_CtrlConnection.GetMediaStatus tdn.TargetDeviceID lu.LUN mn.IdentNumber
                        return ( Some ws, loaded )
                    else
                        return ( None, loaded )
                else
                    return None, false
            ...
            m_PropPage.Dispatcher.InvokeAsync ( fun () ->
                if tdActived && tgLoaded then
                    m_GraphUpdater.Update luStat.Value.ReadBytesCount luStat.Value.WrittenBytesCount luStat.Value.ReadTickCount luStat.Value.WriteTickCount
                else
                    m_GraphUpdater.Clear()
                m_Timer.Start()
            ) |> ignore
    ...
    member private _.ShowConfigValue ( loaded : bool ) ( editmode : bool ) : unit =
        ...
        m_IdentNumberTextBox.Text <- sprintf "%d" ( mediaidx_me.toPrim ( mn :> IMediaNode ).IdentNumber )
        m_MediaNameTextBox.IsEnabled <- editmode && ( not loaded )
        m_MediaNameTextBox.Text <- mn.Values.MediaName
        m_BytesCountTextBox.IsEnabled <- editmode && ( not loaded )
        m_BytesCountTextBox.Text <- sprintf "%d" mn.Values.BytesCount
