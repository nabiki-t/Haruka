...
type LogID =
    ...
    | I_MEDIA_CTRL_REQUESTED                = 0x20006A00
...
type HLogger() =
    ...
    static let m_Messages =
        seq { 
            ( LogID.V_INTERFACE_CALLED,                 "Component interface was invoked. method={0}" )
            ( LogID.V_TRACE,                            "Trace. {0}" )
            ( LogID.V_CONFIG_REG_VALUE,                 "Config file name retrieved from registry. \"{0}\"" )
            ...
            ( LogID.V_CTRL_REQ_NORMAL_END,              "Control request completed normally. Request={0}, Additional Message={1}" )
            ...
            ( LogID.I_PROCESS_STARTED_BY_MANUALLY,      "Haruka main process started manually. It run on foreground mode." )
            ( LogID.I_PROCESS_STARTED_BY_SCM,           "Haruka main process started on SCM. It runs as a Windows service." )
            ( LogID.I_OBJ_INSTANCE_CREATED,             "Object {0} is instantiated. {1}" )
            ( LogID.I_START_WAITING_CONNECTION,         "Waiting for a new connection. IdentNumber={0}, PortNumber={1}" )
            ( LogID.I_NEW_CONNECTION_ADDED_TO_SESSION,  "New connection has been added to the session. I_T Nexus={0}" )
            ( LogID.I_CONNECTION_REPLACED_IN_SESSION,   "Existing connection is replaced in the session. I_T Nexus={0}" )
            ( LogID.I_COMPONENT_INITIALIZED,            "Component {0} is initialized." )
            ( LogID.I_COMPONENT_NOTICE_DESTROY,         "The destroy event was detected. Component={0}" )
            ( LogID.I_FAILED_GET_CONFFILE_IN_REG,       "Failed to retrieve configuration file name from registry. KeyName={0}, message={1}" )
            ( LogID.I_FAILED_GET_FALLBACK_CONFFILE,     "Failed to retrieve fallback configuration file name. message={0}" )
            ( LogID.I_ACCEPT_CONNECTION,                "Accept a connection. Source address={0}, source port={1}." )
            ( LogID.I_AUTHENTICATION_SUCCEED,           "Authentication succeeded. Target name={0}" )
            ( LogID.I_ALL_CONNECTION_CLOSED,            "All connections in the session have been closed. This session will be dropped. I_T Nexus={0}" )
            ...
            ( LogID.I_CONNECTION_REMOVED_IN_SESSION,    "A existing connection is removed in the session. I_T Nexus={0}" )
            ( LogID.I_CONNECTION_ALREADY_REMOVED,       "Removal of existing connection requested, but CID is missing. I_T Nexus={0}" )
            ( LogID.I_ACA_STAT_WAS_DUPLICATED,          "Duplicate ACA status detected; the second one ignored. Sense Key={0}, ASC={1}" )
            ...
            ( LogID.I_PR_FILE_VALIDATE_ERROR,           "Persistent reservation file validation error; ignoring this file. All PRs cleared. File={0}, Message={1}" )
            ( LogID.I_FAILED_LOAD_PR_FILE,              "Failed to load persistent reservation file; ignoring it. All PRs cleared. File={0}, Message={1}" )
            ( LogID.I_SUCCEED_TO_SAVE_PR_FILE,          "Successfully saved persistent reservation file." )
            ( LogID.I_SUCCEED_TO_DELETE_PR_FILE,        "Successfully deleted persistent reservation file." )
            ( LogID.I_PR_FILE_NOT_EXIST,                "Persistent reservation file does not exist. File Name={0}" )
            ...
            ( LogID.I_END_DISCOVERY_SESSION,            "Exited discovery session normally. Initiator Name={0}" )
            ...
            ( LogID.I_KILLED_TARGET_DEVICE_PROC,        "Target device process was killed on user request. Target device ID={0}, Proccess ID={1}" )
            ( LogID.I_TRY_START_INITMEDIA_PROC,         "Try to start media creation process. Arguments={0}" )
            ( LogID.I_KILLED_INITMEDIA_PROC,            "Media creation process was killed on user request. Proccess ID={0}" )
            ...
            ( LogID.I_INITMEDIA_PROC_DLETED_FILE,       "Successfully deleted media file. File name={0}" )
            ...
            ( LogID.I_SESSION_DESTRUCTING,              "Session has been destroyed. TSIH={0}, I_T Nexus={1}" )
            ...
            ( LogID.I_EXCEPTION_IGNORED,                "An exception was ignored. Exception={0}, Message={1}" )
            ...
            ( LogID.I_CREATE_NEW_SESSION,               "New session has been created. I_T Nexus = {0}, TSIH={1}" )
            ( LogID.I_REINSTATE_NEW_SESSION,            "The session has been rebuilt. I_T Nexus = {0}" )
            ( LogID.I_SESSION_REMOVED,                  "The session object has been removed." )
            ...
            ( LogID.I_RESPONSE_WITH_CA_NOT_EST_ACA,     "CA response received; ACA status will not be established." )
            ...
            ( LogID.I_MEDIA_CTRL_REQUESTED,             "Media control request. {0}" )
            ...
            ( LogID.W_NEGOTIATION_RESET,                "Text negotiation has been reset. ITT={0}. Reason={1}" )
            ( LogID.W_OLD_PDU_DELETED,                  "An old PDU with duplicate StatSN ({0}) existed; it was deleted." )
            ...
            ( LogID.W_RESERVATION_CONFLICT,             "Reservation conflict. Commnad={0}, Reason={1}" )
            ...
            ( LogID.W_FAILED_GET_DIR_INFO,              "Failed to retrieve directories information. Path={0}, Message={1}" )
            ...
            ( LogID.W_CONN_REJECTED_DUE_TO_WHITELIST,   "Connection rejected due to whitelist. Source address={0}, source port={1}." )
            ( LogID.W_CONNECTION_ERROR,                 "Communication error occurred. Drop this connection. Message={0}." )
            ( LogID.W_ISCSI_TASK_REMOVED,               "iSCSI task removed. ITT={0}, CmdSN={1}, Reason={2}." )

            ( LogID.E_UNEXPECTED_ERROR,                 "Unexpected error. Exception={0}, message={1}" )
            ...
            ( LogID.E_HEADER_DIGEST_ERROR,              "Header digest error; session recovery will occur." )
            ...
            ( LogID.E_SCSI_ACA_EXCEPTION_RAISED,        "SCSIACAException is raised. SenseKey={0}, ASC/ASCQ={1}, msg={2}" )
            ( LogID.E_UNSUPPORTED_SCSI_COMMAND_VALUE,   "Unsupported SCSI command value was received. msg={0}" )
            ...
            ( LogID.E_TASK_IRREGAL_TERMINATED,          "SCSI task was illegally terminated. Status={0}, msg={1}" )
            ( LogID.E_UNEXPECTED_ACA_EXCEPTION,         "Unexpected ACA exception detected; escalating to session recovery. Message={0}" )
            ( LogID.E_LU_CREATE_RETRY_OVER,             "Retry count exceeded while attempting to create LU." )
            ...
            ( LogID.E_FAILED_RECEIVE_ALL_DATA_OUT_BYTES,"In a SCSI write command, failure to receive all data-out bytes triggers session recovery when ErrorRecoveryLevel=0." )
            ( LogID.E_STATSN_SIGNIFICANTLY_DIFFERENT,   "The current StatSN ({0}) and the initiator's ExpStatSN ({1}) differ significantly; when ErrorRecoveryLevel=0, this triggers session recovery." )
            ...
            ( LogID.E_TD_PROC_RETRY_COUNT_OVER,         "Target device process restart count exceeded. Abort restart. Target Device ID={0}, Last process ID={1}" )
            ( LogID.E_FAILED_START_TD_PROC,             "Failed to start target device process. Target device ID={0}, message={1}" )
            ( LogID.E_FAILED_SEND_REQUEST_TO_TD,        "Failed to send request to target device; terminating the process. Target device ID={0}, Process ID={1}, message={2}" )

            ( LogID.F_INTERNAL_ASSERTION,               "Internal consistency error detected. message={0}" )
            ( LogID.F_UNEXPECTED_ERROR,                 "Unexpected error. Exception={0}, message={1}" )
            ( LogID.F_UNKNOWN_LOG_MESSAGE_ID,           "Unknown log message. ID={0}." )
            ( LogID.F_FAILED_LOAD_CONFIG_FILE,          "Failed to load configuration file. File name={0}, message={1}" )
            ( LogID.F_CONFIG_FILE_VALIDATE_ERROR,       "Configuration file validation error. File name={0}, message={1}" )
            ...
            ( LogID.F_LURESET_REQ_TO_DUMMY_LU,          "LU reset requested for dummy device LU; rebooting the target device." )
        }
