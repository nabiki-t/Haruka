...
// Definition of ConfRW_UT002_TargetGroupID configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_TargetGroupID
...
///  ConfRW_UT002_TargetGroupID class imprements read and write function of configuration.
type ConfRW_UT002_TargetGroupID() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_TargetGroupID.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_TargetGroupID.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                tgid_me.fromString( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
            ...
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( tgid_me.fromString( subElem.Value ) );
            D4 = tgid_me.Zero;
            D5 = tgid_me.fromString( "TG_99000099" );
            D6 = 
                let subElem = elem.Element( XName.Get "D6" )
                if subElem = null then
                    tgid_me.fromString( "TG_98000098" );
                else
                    tgid_me.fromString( subElem.Value );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_TargetGroupID.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_TargetGroupID.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr ( tgid_me.toString (elem.D1) )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr ( tgid_me.toString (itr) )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr ( tgid_me.toString (elem.D3.Value) )
            yield sprintf "%s%s<D6>%s</D6>" singleIndent indentStr ( tgid_me.toString (elem.D6) )
            yield sprintf "%s</%s>" indentStr elemName
        }
