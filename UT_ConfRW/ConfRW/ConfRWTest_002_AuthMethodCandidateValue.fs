...
// Definition of ConfRW_UT002_AuthMethodCandidateValue configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_AuthMethodCandidateValue
...
type [<NoComparison>]T_Test = {
    D1 : AuthMethodCandidateValue;
    D2 : AuthMethodCandidateValue list;
    D3 : AuthMethodCandidateValue option;
    D4 : AuthMethodCandidateValue;
    D5 : AuthMethodCandidateValue;
    D6 : AuthMethodCandidateValue;
}

///  ConfRW_UT002_AuthMethodCandidateValue class imprements read and write function of configuration.
type ConfRW_UT002_AuthMethodCandidateValue() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
      ...
      <xsd:element name='D1' >
        <xsd:simpleType>
          <xsd:restriction base='xsd:string'>
            ...
            <xsd:enumeration value='CHAP' />
            <xsd:enumeration value='SRP' />
            <xsd:enumeration value='KRB5' />
            <xsd:enumeration value='SPKM1' />
            <xsd:enumeration value='SPKM2' />
            <xsd:enumeration value='NotUnderstood' />
          </xsd:restriction>
        </xsd:simpleType>
      </xsd:element>
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_AuthMethodCandidateValue.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_AuthMethodCandidateValue.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                AuthMethodCandidateValue.fromStringValue( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> AuthMethodCandidateValue.fromStringValue( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( AuthMethodCandidateValue.fromStringValue( subElem.Value ) );
            D4 = AuthMethodCandidateValue.AMC_None;
            D5 = AuthMethodCandidateValue.fromStringValue( "SRP" );
            D6 = 
                let subElem = elem.Element( XName.Get "D6" )
                if subElem = null then
                    AuthMethodCandidateValue.fromStringValue( "SPKM1" );
                else
                    AuthMethodCandidateValue.fromStringValue( subElem.Value );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_AuthMethodCandidateValue.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_AuthMethodCandidateValue.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr ( AuthMethodCandidateValue.toStringName (elem.D1) )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr ( AuthMethodCandidateValue.toStringName (itr) )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr ( AuthMethodCandidateValue.toStringName (elem.D3.Value) )
            yield sprintf "%s%s<D6>%s</D6>" singleIndent indentStr ( AuthMethodCandidateValue.toStringName (elem.D6) )
            yield sprintf "%s</%s>" indentStr elemName
        }
