...
// Definition of ConfRW_UT002_ISID configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_ISID
...
///  ConfRW_UT002_ISID class imprements read and write function of configuration.
type ConfRW_UT002_ISID() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_ISID.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_ISID.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                isid_me.HexStringToISID( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> isid_me.HexStringToISID( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( isid_me.HexStringToISID( subElem.Value ) );
            D4 = isid_me.zero;
            D5 = isid_me.HexStringToISID( "0x1234567890AB" );
            D7 = 
                let subElem = elem.Element( XName.Get "D7" )
                if subElem = null then
                    isid_me.HexStringToISID( "0x0123456789AB" );
                else
                    isid_me.HexStringToISID( subElem.Value );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_ISID.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_ISID.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr ( isid_me.toString (elem.D1) )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr ( isid_me.toString (itr) )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr ( isid_me.toString (elem.D3.Value) )
            yield sprintf "%s%s<D7>%s</D7>" singleIndent indentStr ( isid_me.toString (elem.D7) )
            yield sprintf "%s</%s>" indentStr elemName
        }
