...
type CommandRunner_Test2() =
    ...
    member _.set_BlockDeviceLU_001 ( cmdstr : string ) =
        ...
        ss.p_UpdateBlockDeviceLUNode <- ( fun argnode arglun argname argmm argotl ->
            flg1 <- true
            Assert.True(( arglun = lun_me.fromPrim 1234605616436508552UL ))
            initnode.CreateUpdatedNode arglun argname argmm argotl
        )
        ss.p_CheckTargetGroupUnloaded <- ( fun cc node -> Task.FromResult () )
        ...
        Assert.True(( ( r_cn :?> ILUNode ).LUName = "" ))
        Assert.True(( ( r_cn :?> ILUNode ).MaxMultiplicity = Constants.LU_DEF_MULTIPLICITY ))
        Assert.True(( ( r_cn :?> ConfNode_BlockDeviceLU ).OptimalTransferLength = blkcnt_me.ofUInt32 Constants.LU_DEF_OPTIMAL_TRANSFER_LENGTH ))
        Assert.True(( flg1 ))
        let out_rs = CheckOutputMessage out_ms out_ws "LU" ""
        GlbFunc.AllDispose [ in_ws; in_rs; in_ms; out_ws; out_rs; out_ms; ]
    ...
    [<Theory>]
    [<InlineData( "set OPTIMALTRANSFERLENGTH 200" )>]
    member _.set_BlockDeviceLU_008 ( cmdstr : string ) =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( cmdstr )
        let initnode = CommandRunner_Test1.m_BlockDeviceLUNode :?> ConfNode_BlockDeviceLU
        let mutable flg1 = false
        ...
        ss.p_UpdateBlockDeviceLUNode <- ( fun argnode arglun argname argmm argotl ->
            flg1 <- true
            Assert.True(( argotl = blkcnt_me.ofUInt32 200u ))
            initnode.CreateUpdatedNode arglun argname argmm argotl
        )
        ss.p_CheckTargetGroupUnloaded <- ( fun cc node -> Task.FromResult () )
        ...
        Assert.True(( ( r_cn :?> ILUNode ).LUName = "" ))
        Assert.True(( ( r_cn :?> ILUNode ).MaxMultiplicity = Constants.LU_DEF_MULTIPLICITY ))
        Assert.True(( ( r_cn :?> ConfNode_BlockDeviceLU ).OptimalTransferLength = blkcnt_me.ofUInt32 200u ))
        Assert.True(( flg1 ))
        let out_rs = CheckOutputMessage out_ms out_ws "LU" ""
        GlbFunc.AllDispose [ in_ws; in_rs; in_ms; out_ws; out_rs; out_ms; ]

    [<Theory>]
    [<InlineData( "set OPTIMALTRANSFERLENGTH -1" )>]
    [<InlineData( "set OPTIMALTRANSFERLENGTH aaaaa" )>]
    [<InlineData( "set OPTIMALTRANSFERLENGTH 4294967296" )>]
    member _.set_BlockDeviceLU_009 ( cmdstr : string ) =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( cmdstr )
        let initnode = CommandRunner_Test1.m_BlockDeviceLUNode :?> ConfNode_BlockDeviceLU
        let r = CallCommandLoop cr ( Some ( ss, cc, initnode ) )
        Assert.True(( r = ( true, Some( ss, cc, initnode ) ) ))
    ...
    [<Theory>]
    [<InlineData( "set aaaa -1" )>]
    member _.set_BlockDeviceLU_010 ( cmdstr : string ) =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( cmdstr )
        let initnode = CommandRunner_Test1.m_BlockDeviceLUNode :?> ConfNode_BlockDeviceLU
        let r = CallCommandLoop cr ( Some ( ss, cc, initnode ) )
        Assert.True(( r = ( true, Some( ss, cc, initnode ) ) ))
    ...
    [<Theory>]
    [<InlineData( "4096", "set BLOCKSIZE 512", "512" )>]
    [<InlineData( "512", "set BLOCKSIZE 4096", "4096" )>]
    member _.set_MemBufferMedia_006 ( bs1 : string ) ( cmdstr : string ) ( bs2 : string ) =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( cmdstr )
        let wnode = CommandRunner_Test1.m_MemBufferMediaNode :?> ConfNode_MemBufferMedia
        let conf : TargetGroupConf.T_MemBuffer = {
            IdentNumber = mediaidx_me.fromPrim 0u;
            MediaName = "";
            BytesCount = 8192UL;
            BlockSize = Blocksize.fromStringValue bs1;
        }
        let initnode = wnode.CreateUpdatedNode conf
        let mutable flg1 = false
        ...
        ss.p_UpdateMemBufferMediaNode <- ( fun argcn argconf ->
            flg1 <- true
            Assert.True(( argcn = initnode ))
            Assert.True(( argconf.BlockSize = Blocksize.fromStringValue bs2 ))
            initnode.CreateUpdatedNode argconf
        )
        ss.p_CheckTargetGroupUnloaded <- ( fun cc node -> Task.FromResult () )
        ...
        let expconf = {
            conf with
                BlockSize = Blocksize.fromStringValue bs2;
        }
        Assert.True(( ( r_cn :?> IMediaNode ).MediaConfData = TargetGroupConf.U_MemBuffer( expconf ) ))
        Assert.True(( flg1 ))
        let out_rs = CheckOutputMessage out_ms out_ws "MD" ""
    ...
    [<Theory>]
    [<InlineData( "set BLOCKSIZE -1" )>]
    [<InlineData( "set BLOCKSIZE 0" )>]
    [<InlineData( "set BLOCKSIZE 511" )>]
    [<InlineData( "set BLOCKSIZE 513" )>]
    [<InlineData( "set BLOCKSIZE 4095" )>]
    [<InlineData( "set BLOCKSIZE 4097" )>]
    [<InlineData( "set BLOCKSIZE aaa" )>]
    member _.set_MemBufferMedia_007 ( cmdstr : string ) =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( cmdstr )
        let initnode = CommandRunner_Test1.m_MemBufferMediaNode :?> ConfNode_MemBufferMedia
        let r = CallCommandLoop cr ( Some ( ss, cc, initnode ) )
        Assert.True(( r = ( true, Some( ss, cc, initnode ) ) ))
    ...
    [<Theory>]
    [<InlineData( "set aaa -1" )>]
    member _.set_MemBufferMedia_008 ( cmdstr : string ) =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( cmdstr )
        let initnode = CommandRunner_Test1.m_MemBufferMediaNode :?> ConfNode_MemBufferMedia
        let r = CallCommandLoop cr ( Some ( ss, cc, initnode ) )
        Assert.True(( r = ( true, Some( ss, cc, initnode ) ) ))
    ...
    [<Fact>]
    member _.StatusAll_Controller_MotModified_001 () =
    ...
    [<Fact>]
    member _.StatusAll_Controller_Modified_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
    ...
    [<Fact>]
    member _.StatusAll_TargetDevice_Running_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cn = CommandRunner_Test1.m_ControllerNode :?> ConfNode_Controller
    ...
    [<Fact>]
    member _.StatusAll_TargetDevice_Running_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cn = CommandRunner_Test1.m_ControllerNode :?> ConfNode_Controller
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tdn2 = tdn.SetModified()
    ...
    [<Fact>]
    member _.StatusAll_TargetDevice_Unloaded_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cn = CommandRunner_Test1.m_ControllerNode :?> ConfNode_Controller
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
    ...
    [<Fact>]
    member _.StatusAll_TargetDevice_Unloaded_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cn = CommandRunner_Test1.m_ControllerNode :?> ConfNode_Controller
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tdn2 = ( tdn :> IConfigFileNode ).ResetModifiedFlag() :?> ConfNode_TargetDevice
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Active_001 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        ...
        let lines = [|
            for i = 1 to 3 do
                yield ( out_rs.ReadLine() ).TrimStart()
        |]
        Assert.StartsWith( "ACTIVE ", lines.[2] )
        Assert.True( flg1 )
        Assert.True( flg2 )
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Active_Mod_001 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        let cn = new ConfNode_Controller( st, cnr, cnr.NextID ) :> IConfigureNode
        let tdn =
            ...
            new ConfNode_TargetDevice( st, cnr, cnr.NextID, GlbFunc.newTargetDeviceID(), conf ) :> IConfigureNode
        let tgn =
            new ConfNode_TargetGroup( st, cnr, cnr.NextID, GlbFunc.newTargetGroupID(), "", true, ModifiedStatus.Modified )
        let mutable flg1 = false
        let mutable flg2 = false
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Loaded_001 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        let cn = new ConfNode_Controller( st, cnr, cnr.NextID ) :> IConfigureNode
        ...
        let lines = [|
            for i = 1 to 3 do
                yield ( out_rs.ReadLine() ).TrimStart()
        |]
        Assert.StartsWith( "LOADED ", lines.[2] )
        Assert.True( flg1 )
        Assert.True( flg2 )
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Loaded_Mod_001 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        let cn = new ConfNode_Controller( st, cnr, cnr.NextID ) :> IConfigureNode
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Unloaded_Mod_001 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        let cn = new ConfNode_Controller( st, cnr, cnr.NextID ) :> IConfigureNode
        ...
        let lines = [|
            for i = 1 to 3 do
                yield ( out_rs.ReadLine() ).TrimStart()
        |]
        Assert.StartsWith( "UNLOADED(MOD)", lines.[2] )
        Assert.True( flg1 )
        Assert.True( flg2 )
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Unloaded_001 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        let cn = new ConfNode_Controller( st, cnr, cnr.NextID ) :> IConfigureNode
    ...
    [<Fact>]
    member _.StatusAll_TargetGroup_Unloaded_002 () =
        let st = StringTable( "" )
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "statusall" )
        let cnr = new ConfNodeRelation()
        let cn = new ConfNode_Controller( st, cnr, cnr.NextID ) :> IConfigureNode
        ...
        ss.p_ControllerNode <- ( cn :?> ConfNode_Controller )
        ss.p_GetTargetDeviceNodes <- ( fun () -> [ tdn :?> ConfNode_TargetDevice ] )
        cc.p_GetTargetDeviceProcs <- ( fun () -> Task.FromResult [] )   // Target device procs missing
        cc.p_GetActiveTargetGroups <- ( fun _ -> task { flg1 <- true; return [] } )
        cc.p_GetLoadedTargetGroups <- ( fun _ -> task { flg2 <- true; return [] } )
    ...
    [<Fact>]
    member _.Status_Controller_NotModified_001 () =
    ...
    [<Fact>]
    member _.Status_Controller_Modified_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode :?> ConfNode_Controller
        let conf : HarukaCtrlConf.T_HarukaCtrl = {
            RemoteCtrl = Some cn.RemoteCtrlValue;
    ...
    [<Fact>]
    member _.Status_TargetDevice_Running_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
    ...
    [<Fact>]
    member _.Status_TargetDevice_Running_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn =
            CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
    ...
    [<Fact>]
    member _.Status_TargetDevice_Unloaded_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tdn2 = tdn.CreateUpdatedNode tdn.TargetDeviceID tdn.TargetDeviceName tdn.EnableStatSNAckChecker tdn.NegotiableParameters tdn.LogParameters
    ...
    [<Fact>]
    member _.Status_TargetGroup_Active_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tgn = CommandRunner_Test1.m_TargetGroupNode :?> ConfNode_TargetGroup
    ...
    [<Fact>]
    member _.Status_TargetGroup_Active_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tgn = CommandRunner_Test1.m_TargetGroupNode :?> ConfNode_TargetGroup
    ...
    [<Fact>]
    member _.Status_TargetGroup_Loaded_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tgn = CommandRunner_Test1.m_TargetGroupNode :?> ConfNode_TargetGroup
    ...
    [<Fact>]
    member _.Status_TargetGroup_Loaded_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tgn = CommandRunner_Test1.m_TargetGroupNode :?> ConfNode_TargetGroup
    ...
    [<Fact>]
    member _.Status_TargetGroup_Unloaded_Mod_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tgn = CommandRunner_Test1.m_TargetGroupNode :?> ConfNode_TargetGroup
    ...
    [<Fact>]
    member _.Status_TargetGroup_Unloaded_001 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
    ...
    [<Fact>]
    member _.Status_TargetGroup_Unloaded_002 () =
        let in_ms, in_ws, in_rs, out_ms, out_ws, cr, ss, cc = GenStub( "status" )
        let cn = CommandRunner_Test1.m_ControllerNode
        let tdn = CommandRunner_Test1.m_TargetDeviceNode :?> ConfNode_TargetDevice
        let tgn = CommandRunner_Test1.m_TargetGroupNode :?> IConfigFileNode
        ...
        cc.p_GetTargetDeviceProcs <- ( fun () -> Task.FromResult [] )   // Target Device Proc missing
        ss.p_GetAncestorTargetGroup <- ( fun argnode ->
            Assert.Same( argnode, tgn2 )
            Some tgn2
        )
