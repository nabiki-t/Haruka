...
type ClientConfigTest() =
    ...
    let SendReadCapacityCDB ( sess : iSCSI_Initiator ) ( lun : LUN_T ) : Task< struct( uint64 * uint32 ) > =
        task {
            ...
            |]
            let cid = sess.CID.[0]
            let! struct( itt1, cmdsn1 ) = sess.SendSCSICommandPDU cid BitI.F BitF.T BitR.T BitW.F TaskATTRCd.SIMPLE_TASK lun 32u cdb PooledBuffer.Empty 0u
    ...
    member _.Test() =
        task {
            ...
            client.RunCommand "create membuffer 65536" "Created" "MD> "
            client.RunCommand "select 0" "" "MD> "
            ...
            let! _ = r1.SendLogoutRequestPDU r1.CID.[0] BitI.F LogoutReqReasonCd.CLOSE_SESS r1.CID.[0]
            ...
            client.RunCommand "create debug" "Created" "LU> "
            client.RunCommand "select 0" "" "MD> "
            client.RunCommand "create membuffer 32768" "Created" "MD> "
            client.RunCommand "select 0" "" "MD> "
            client.RunCommand ( sprintf "set BlockSize %d" blockSize ) "" "MD> "
            client.RunCommand "unselect" "" "MD> "
            ...
            // logout
            let! _ = r3.SendLogoutRequestPDU r3.CID.[0] BitI.F LogoutReqReasonCd.CLOSE_SESS r3.CID.[0]
            ...
            client.RunCommand "create debug" "Created" "LU> "
            client.RunCommand "select 0" "" "MD> "
            client.RunCommand "create membuffer 16384" "Created" "MD> "
            client.RunCommand "select 0" "" "MD> "
            client.RunCommand ( sprintf "set BlockSize %d" blockSize ) "" "MD> "
            client.RunCommand "unselect" "" "MD> "
            client.RunCommand "validate" "All configurations are vlidated" "MD> "
