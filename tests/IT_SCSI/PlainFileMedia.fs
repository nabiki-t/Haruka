...
// PlainFileMedia.fs : Test cases for plain file media.
//
...
[<CollectionDefinition( "SCSI_PlainFileMedia" )>]
type SCSI_PlainFileMedia_Fixture() =

    static let m_WorkPath = Functions.AppendPathName ( Path.GetTempPath() ) ( Guid.NewGuid().ToString( "N" ) )
    static let m_BlockSize1 = 512
    static let m_MediaSize1 = uint32 m_BlockSize1 * 1048576u    // 512MB
    static let m_MediaFile1 = Path.Combine( m_WorkPath, "MediaFile1.txt" )

    static let m_BlockSize2 = 4096
    static let m_MediaSize2 = uint32 m_BlockSize2 * 131072u     // 512MB
    static let m_MediaFile2 = Path.Combine( m_WorkPath, "MediaFile2.txt" )

    let m_iSCSIPortNo = GlbFunc.nextTcpPortNo()

    // Add default configurations
    let AddDefaultConf( client : ClientProc ): unit =
        let fs1 = File.OpenWrite m_MediaFile1
        fs1.SetLength( int64 m_MediaSize1 )
        fs1.Close()
        fs1.Dispose()

        let fs2 = File.OpenWrite m_MediaFile2
        fs2.SetLength( int64 m_MediaSize2 )
        fs2.Close()
        fs2.Dispose()
        ...
        client.RunCommand ( sprintf "create plainfile /n %s" m_MediaFile1 ) "Created" "LU> "
        ...
        client.RunCommand ( sprintf "create plainfile /n %s" m_MediaFile2 ) "Created" "LU> "
        client.RunCommand "select 0" "" "MD> "
    ...
    interface ICollectionFixture<SCSI_PlainFileMedia_Fixture>
...
[<Collection( "SCSI_PlainFileMedia" )>]
type SCSI_PlainFileMedia( fx : SCSI_PlainFileMedia_Fixture ) =
    ...
    static let m_MediaSize1 = SCSI_PlainFileMedia_Fixture.MediaSize1
    static let m_BlockSize1 = SCSI_PlainFileMedia_Fixture.BlockSize1
    static let m_BlockCount1 = m_MediaSize1 / ( Blocksize.toUInt32 m_BlockSize1 )

    static let m_MediaSize2 = SCSI_PlainFileMedia_Fixture.MediaSize2
    static let m_BlockSize2 = SCSI_PlainFileMedia_Fixture.BlockSize2
    ...
    member _.Format_001 () =
        task {
            ...
            for i = 0 to v2.Length - 1 do
                Assert.StrictEqual( v.Array.[i], v2.Array.[i] )
    ...
    member _.TestUnitReady_001 () =
        task {
            ...
            let! itt_f = r.Send_TestUnitReady TaskATTRCd.SIMPLE_TASK g_LUN1 NACA.T
            let! _ = r.WaitSCSIResponseGoodStatus itt_f
