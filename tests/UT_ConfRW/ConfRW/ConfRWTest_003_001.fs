...
namespace Haruka.Test.UT.ConfRW_003_001
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    <xsd:complexType><xsd:sequence>
      <xsd:element name='R019_1' >
        <xsd:complexType><xsd:sequence>
          ...
          <xsd:element name='D2' minOccurs='1' maxOccurs='3' >
            <xsd:simpleType>
              <xsd:restriction base='xsd:int'>
              </xsd:restriction>
            </xsd:simpleType>
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            R019_1 =
                ReaderWriter.Read_T_R019_1( elem.Element( XName.Get "R019_1" ) );
        }

    /// <summary>
    ///  Read T_R019_1 data from XML document.
    /// </summary>
    /// <param name="elem">
    ///  Loaded XML document.
    /// </param>
    /// <returns>
    ///  parsed T_R019_1 data structure.
    /// </returns>
    static member private Read_T_R019_1 ( elem : XElement ) : T_R019_1 = 
        {
            D1 =
    ...
    static member private T_Test_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield! ReaderWriter.T_R019_1_toString ( indent + 1 ) indentStep ( elem.R019_1 ) "R019_1"
            yield sprintf "%s</%s>" indentStr elemName
        }

    /// <summary>
    ///  Write T_R019_1 data structure to configuration file.
    /// </summary>
    /// <param name="indent">
    ///  Indent space count.
    /// </param>
    ...
    ///  Array of the generated string.
    /// </returns>
    static member private T_R019_1_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_R019_1 ) ( elemName : string ) : seq<string> = 
        let indentStr = String.replicate ( indent * indentStep ) " "
        let singleIndent = String.replicate ( indentStep ) " "
        seq {
            yield sprintf "%s<%s>" indentStr elemName
            yield sprintf "%s%s<D1>%d</D1>" singleIndent indentStr (elem.D1)
            if elem.D2.Length < 1 || elem.D2.Length > 3 then 
