...
// Definition of ConfRW_UT002_Blocksize configuration reader/writer function.

//=============================================================================
// Namespace declaration

namespace Haruka.Test.UT.ConfRW_002_Blocksize
...
///  ConfRW_UT002_Blocksize class imprements read and write function of configuration.
type ConfRW_UT002_Blocksize() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_Blocksize.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_Blocksize.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                Blocksize.fromStringValue( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> Blocksize.fromStringValue( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( Blocksize.fromStringValue( subElem.Value ) );
            D4 = Blocksize.BS_512;
            D5 = Blocksize.fromStringValue( "512" );
            D6 = Blocksize.fromStringValue( "512" );
            D7 = 
                let subElem = elem.Element( XName.Get "D7" )
                if subElem = null then
                    Blocksize.fromStringValue( "4096" );
                else
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_Blocksize.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_Blocksize.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr ( Blocksize.toStringName (elem.D1) )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr ( Blocksize.toStringName (itr) )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr ( Blocksize.toStringName (elem.D3.Value) )
            yield sprintf "%s%s<D7>%s</D7>" singleIndent indentStr ( Blocksize.toStringName (elem.D7) )
            yield sprintf "%s%s<D8>%s</D8>" singleIndent indentStr ( Blocksize.toStringName (elem.D8) )
            yield sprintf "%s</%s>" indentStr elemName
        }
