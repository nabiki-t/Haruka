...
// Definition of ConfRW_UT002_LogLevel configuration reader/writer function.

//=============================================================================
// Namespace declaration

namespace Haruka.Test.UT.ConfRW_002_LogLevel
...
///  ConfRW_UT002_LogLevel class imprements read and write function of configuration.
type ConfRW_UT002_LogLevel() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_LogLevel.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_LogLevel.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                LogLevel.fromString( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> LogLevel.fromString( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( LogLevel.fromString( subElem.Value ) );
            D4 = LogLevel.LOGLEVEL_INFO;
            D5 = LogLevel.fromString( "WARNING" );
            D6 = 
                let subElem = elem.Element( XName.Get "D6" )
                if subElem = null then
                    LogLevel.fromString( "VERBOSE" );
                else
                    LogLevel.fromString( subElem.Value );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_LogLevel.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_LogLevel.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr ( LogLevel.toString (elem.D1) )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr ( LogLevel.toString (itr) )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr ( LogLevel.toString (elem.D3.Value) )
            yield sprintf "%s%s<D6>%s</D6>" singleIndent indentStr ( LogLevel.toString (elem.D6) )
            yield sprintf "%s</%s>" indentStr elemName
        }
