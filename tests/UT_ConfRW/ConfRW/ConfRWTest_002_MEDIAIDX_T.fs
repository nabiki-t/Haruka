...
// Definition of ConfRW_UT002_MEDIAIDX_T configuration reader/writer function.

//=============================================================================
// Namespace declaration

namespace Haruka.Test.UT.ConfRW_002_MEDIAIDX_T
...
///  ConfRW_UT002_MEDIAIDX_T class imprements read and write function of configuration.
type ConfRW_UT002_MEDIAIDX_T() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_MEDIAIDX_T.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_MEDIAIDX_T.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                mediaidx_me.fromPrim( UInt32.Parse( elem.Element( XName.Get "D1" ).Value ) );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> mediaidx_me.fromPrim( UInt32.Parse( itr.Value ) ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( mediaidx_me.fromPrim( UInt32.Parse( subElem.Value ) ) );
            D4 = mediaidx_me.fromPrim( 0u );
            D5 = mediaidx_me.fromPrim( 99u );
            D6 = mediaidx_me.fromPrim( 16u );
            D7 = 
                let subElem = elem.Element( XName.Get "D7" )
                if subElem = null then
                    mediaidx_me.fromPrim( 98u );
                else
                    mediaidx_me.fromPrim( UInt32.Parse( subElem.Value ) );
            D8 = 
                let subElem = elem.Element( XName.Get "D8" )
                if subElem = null then
                    mediaidx_me.fromPrim( 16u );
                else
                    mediaidx_me.fromPrim( UInt32.Parse( subElem.Value ) );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_MEDIAIDX_T.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_MEDIAIDX_T.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%d</D1>" singleIndent indentStr ( mediaidx_me.toPrim (elem.D1) )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%d</D2>" singleIndent indentStr ( mediaidx_me.toPrim (itr) )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%d</D3>" singleIndent indentStr ( mediaidx_me.toPrim (elem.D3.Value) )
            yield sprintf "%s%s<D7>%d</D7>" singleIndent indentStr ( mediaidx_me.toPrim (elem.D7) )
            yield sprintf "%s%s<D8>%d</D8>" singleIndent indentStr ( mediaidx_me.toPrim (elem.D8) )
            yield sprintf "%s</%s>" indentStr elemName
        }
