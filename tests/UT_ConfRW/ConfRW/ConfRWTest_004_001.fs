...
namespace Haruka.Test.UT.ConfRW_004_001
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    <xsd:complexType><xsd:sequence>
      <xsd:element name='C' minOccurs='16' maxOccurs='65535' >
        <xsd:simpleType>
          <xsd:restriction base='xsd:int'>
          </xsd:restriction>
        </xsd:simpleType>
      </xsd:element>
      <xsd:element name='D' minOccurs='1' maxOccurs='16' >
        <xsd:simpleType>
          <xsd:restriction base='xsd:int'>
          </xsd:restriction>
        </xsd:simpleType>
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            C =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "C" )
                |> Seq.map ( fun itr -> Int32.Parse( itr.Value ) )
                |> Seq.toList
            D =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D" )
                |> Seq.map ( fun itr -> Int32.Parse( itr.Value ) )
                |> Seq.toList
        }
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if elem.C.Length < 16 || elem.C.Length > 65535 then 
                raise <| ConfRWException( "Element count restriction error. C" )
            for itr in elem.C do
                yield sprintf "%s%s<C>%d</C>" singleIndent indentStr (itr)
            if elem.D.Length < 1 || elem.D.Length > 16 then 
                raise <| ConfRWException( "Element count restriction error. D" )
            for itr in elem.D do
                yield sprintf "%s%s<D>%d</D>" singleIndent indentStr (itr)
            yield sprintf "%s</%s>" indentStr elemName
        }
