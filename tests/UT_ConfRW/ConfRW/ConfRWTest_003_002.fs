...
namespace Haruka.Test.UT.ConfRW_003_002
...
type [<NoComparison>]T_Test = {
    R020_1 : T_R020_1 list;
}

and [<NoComparison>]T_R020_1 = {
    D1 : int32;
    D2 : int32;
}
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    <xsd:complexType><xsd:sequence>
      <xsd:element name='R020_1' minOccurs='1' maxOccurs='3' >
        <xsd:complexType><xsd:sequence>
          <xsd:element name='D1' >
            <xsd:simpleType>
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            R020_1 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "R020_1" )
                |> Seq.map ( fun itr -> ReaderWriter.Read_T_R020_1 itr )
                |> Seq.toList
        }

    /// <summary>
    ///  Read T_R020_1 data from XML document.
    /// </summary>
    /// <param name="elem">
    ///  Loaded XML document.
    /// </param>
    /// <returns>
    ///  parsed T_R020_1 data structure.
    /// </returns>
    static member private Read_T_R020_1 ( elem : XElement ) : T_R020_1 = 
        {
            D1 =
    ...
    static member private T_Test_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if elem.R020_1.Length < 1 || elem.R020_1.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. R020_1" )
            for itr in elem.R020_1 do
                yield! ReaderWriter.T_R020_1_toString ( indent + 1 ) indentStep itr "R020_1"
            yield sprintf "%s</%s>" indentStr elemName
        }

    /// <summary>
    ///  Write T_R020_1 data structure to configuration file.
    /// </summary>
    /// <param name="indent">
    ///  Indent space count.
    /// </param>
    ...
    ///  Array of the generated string.
    /// </returns>
    static member private T_R020_1_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_R020_1 ) ( elemName : string ) : seq<string> = 
        let indentStr = String.replicate ( indent * indentStep ) " "
        let singleIndent = String.replicate ( indentStep ) " "
        seq {
            yield sprintf "%s<%s>" indentStr elemName
