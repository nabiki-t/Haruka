...
// Definition of ConfRW_UT002_LUN_T configuration reader/writer function.

//=============================================================================
// Namespace declaration

namespace Haruka.Test.UT.ConfRW_002_LUN_T
...
///  ConfRW_UT002_LUN_T class imprements read and write function of configuration.
type ConfRW_UT002_LUN_T() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_LUN_T.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_LUN_T.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                lun_me.fromStringValue( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
            ...
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( lun_me.fromStringValue( subElem.Value ) );
            D4 = lun_me.zero;
            D5 = lun_me.fromPrim( 99UL );
            D6 = 
                let subElem = elem.Element( XName.Get "D6" )
                if subElem = null then
                    lun_me.fromPrim( 98UL );
                else
                    lun_me.fromStringValue( subElem.Value );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_LUN_T.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_LUN_T.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if lun_me.toPrim (elem.D1) < 0UL then
                raise <| ConfRWException( "Min value(LUN_T) restriction error. D1" )
            if lun_me.toPrim (elem.D1) > 255UL then
                raise <| ConfRWException( "Max value(LUN_T) restriction error. D1" )
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr ( lun_me.toString (elem.D1) )
            ...
            for itr in elem.D2 do
                if lun_me.toPrim (itr) < 0UL then
                    raise <| ConfRWException( "Min value(LUN_T) restriction error. D2" )
                if lun_me.toPrim (itr) > 255UL then
                    raise <| ConfRWException( "Max value(LUN_T) restriction error. D2" )
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr ( lun_me.toString (itr) )
            if elem.D3.IsSome then
                if lun_me.toPrim (elem.D3.Value) < 0UL then
                    raise <| ConfRWException( "Min value(LUN_T) restriction error. D3" )
                if lun_me.toPrim (elem.D3.Value) > 255UL then
                    raise <| ConfRWException( "Max value(LUN_T) restriction error. D3" )
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr ( lun_me.toString (elem.D3.Value) )
            if lun_me.toPrim (elem.D6) < 0UL then
                raise <| ConfRWException( "Min value(LUN_T) restriction error. D6" )
            if lun_me.toPrim (elem.D6) > 255UL then
                raise <| ConfRWException( "Max value(LUN_T) restriction error. D6" )
            yield sprintf "%s%s<D6>%s</D6>" singleIndent indentStr ( lun_me.toString (elem.D6) )
            yield sprintf "%s</%s>" indentStr elemName
        }
