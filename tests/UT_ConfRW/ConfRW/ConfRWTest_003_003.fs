...
namespace Haruka.Test.UT.ConfRW_003_003
...
type [<NoComparison>]T_Test = {
    R021_1 : T_R021_1 option;
}

and [<NoComparison>]T_R021_1 = {
    D1 : int32;
    D2 : int32;
}
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    <xsd:complexType><xsd:sequence>
      <xsd:element name='R021_1' minOccurs='0' maxOccurs='1' >
        <xsd:complexType><xsd:sequence>
          <xsd:element name='D1' >
            <xsd:simpleType>
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            R021_1 = 
                let subElem = elem.Element( XName.Get "R021_1" )
                if subElem = null then
                    None
                else
                    Some( ReaderWriter.Read_T_R021_1 subElem );
        }

    /// <summary>
    ///  Read T_R021_1 data from XML document.
    /// </summary>
    /// <param name="elem">
    ///  Loaded XML document.
    /// </param>
    /// <returns>
    ///  parsed T_R021_1 data structure.
    /// </returns>
    static member private Read_T_R021_1 ( elem : XElement ) : T_R021_1 = 
        {
            D1 =
    ...
    static member private T_Test_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if elem.R021_1.IsSome then
                yield! ReaderWriter.T_R021_1_toString ( indent + 1 ) indentStep ( elem.R021_1.Value ) "R021_1"
            yield sprintf "%s</%s>" indentStr elemName
        }

    /// <summary>
    ///  Write T_R021_1 data structure to configuration file.
    /// </summary>
    /// <param name="indent">
    ///  Indent space count.
    /// </param>
    ...
    ///  Array of the generated string.
    /// </returns>
    static member private T_R021_1_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_R021_1 ) ( elemName : string ) : seq<string> = 
        let indentStr = String.replicate ( indent * indentStep ) " "
        let singleIndent = String.replicate ( indentStep ) " "
        seq {
            yield sprintf "%s<%s>" indentStr elemName
