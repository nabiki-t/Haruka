...
// Definition of ConfRW_UT002_CtrlSessionID configuration reader/writer function.

//=============================================================================
// Namespace declaration

namespace Haruka.Test.UT.ConfRW_002_CtrlSessionID
...
type [<NoComparison>]T_Test = {
    D1 : CtrlSessionID;
    D2 : CtrlSessionID list;
    D3 : CtrlSessionID option;
    D4 : CtrlSessionID;
    D5 : CtrlSessionID;
    D6 : CtrlSessionID;
}
...
///  ConfRW_UT002_CtrlSessionID class imprements read and write function of configuration.
type ConfRW_UT002_CtrlSessionID() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_CtrlSessionID.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_CtrlSessionID.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                new CtrlSessionID( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> new CtrlSessionID( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( new CtrlSessionID( subElem.Value ) );
            D4 = CtrlSessionID.Zero;
            D5 = new CtrlSessionID( "CSI_99000000-0000-0000-0000-000000000099" );
            D6 = 
                let subElem = elem.Element( XName.Get "D6" )
                if subElem = null then
                    new CtrlSessionID( "CSI_98000000-0000-0000-0000-000000000098" );
                else
                    new CtrlSessionID( subElem.Value );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_CtrlSessionID.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_CtrlSessionID.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            let work = elem.D1
            yield sprintf "%s%s<D1>%s</D1>" singleIndent indentStr (work.ToString()) 
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                let work = itr
                yield sprintf "%s%s<D2>%s</D2>" singleIndent indentStr (work.ToString()) 
            if elem.D3.IsSome then
                let work = elem.D3.Value
                yield sprintf "%s%s<D3>%s</D3>" singleIndent indentStr (work.ToString()) 
            let work = elem.D6
            yield sprintf "%s%s<D6>%s</D6>" singleIndent indentStr (work.ToString()) 
            yield sprintf "%s</%s>" indentStr elemName
        }
