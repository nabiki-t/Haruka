...
// Definition of ConfRW_UT002_long configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_long
...
///  ConfRW_UT002_long class imprements read and write function of configuration.
type ConfRW_UT002_long() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_long.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_long.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                Int64.Parse( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> Int64.Parse( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( Int64.Parse( subElem.Value ) );
            D4 = 0L;
            D5 = 99L;
            D6 = 16L;
            D7 = 
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_long.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_long.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if (elem.D1) < 1L then
                raise <| ConfRWException( "Min value(long) restriction error. D1" )
            if (elem.D1) > 2L then
                raise <| ConfRWException( "Max value(long) restriction error. D1" )
