...
// Definition of ConfRW_UT002_short configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_short
...
///  ConfRW_UT002_short class imprements read and write function of configuration.
type ConfRW_UT002_short() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_short.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_short.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                Int16.Parse( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> Int16.Parse( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( Int16.Parse( subElem.Value ) );
            D4 = 0s;
            D5 = 99s;
            D6 = 16s;
            D7 = 
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_short.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_short.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if (elem.D1) < 1s then
                raise <| ConfRWException( "Min value(short) restriction error. D1" )
            if (elem.D1) > 2s then
                raise <| ConfRWException( "Max value(short) restriction error. D1" )
