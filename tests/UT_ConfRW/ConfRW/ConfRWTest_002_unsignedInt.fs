...
// Definition of ConfRW_UT002_unsignedInt configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_unsignedInt
...
///  ConfRW_UT002_unsignedInt class imprements read and write function of configuration.
type ConfRW_UT002_unsignedInt() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_unsignedInt.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_unsignedInt.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                UInt32.Parse( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> UInt32.Parse( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( UInt32.Parse( subElem.Value ) );
            D4 = 0u;
            D5 = 99u;
            D6 = 16u;
            D7 = 
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_unsignedInt.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_unsignedInt.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if (elem.D1) < 1u then
                raise <| ConfRWException( "Min value(unsignedInt) restriction error. D1" )
            if (elem.D1) > 2u then
                raise <| ConfRWException( "Max value(unsignedInt) restriction error. D1" )
