...
// Definition of ConfRW_UT002_double configuration reader/writer function.

//=============================================================================
// Namespace declaration

namespace Haruka.Test.UT.ConfRW_002_double
...
///  ConfRW_UT002_double class imprements read and write function of configuration.
type ConfRW_UT002_double() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_double.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_double.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                Double.Parse( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> Double.Parse( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( Double.Parse( subElem.Value ) );
            D4 = 0.0;
            D5 = 99;
            D6 = 16;
            D7 = 
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_double.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_double.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            if (elem.D1) < 1 then
                raise <| ConfRWException( "Min value(double) restriction error. D1" )
            if (elem.D1) > 2 then
                raise <| ConfRWException( "Max value(double) restriction error. D1" )
            yield sprintf "%s%s<D1>%f</D1>" singleIndent indentStr (elem.D1)
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%f</D2>" singleIndent indentStr (itr)
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%f</D3>" singleIndent indentStr (elem.D3.Value)
            yield sprintf "%s%s<D7>%f</D7>" singleIndent indentStr (elem.D7)
            yield sprintf "%s%s<D8>%f</D8>" singleIndent indentStr (elem.D8)
            yield sprintf "%s</%s>" indentStr elemName
        }
