...
namespace Haruka.Test.UT.ConfRW_003_004
...
and [<NoComparison>]T_R022_1 = {
    D1 : int32;
    R022_2 : T_R022_2;
    D2 : int32;
}

and [<NoComparison>]T_R022_2 = {
    D1_1 : int32;
    D1_2 : int32;
}
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            R022_1 =
                ReaderWriter.Read_T_R022_1( elem.Element( XName.Get "R022_1" ) );
        }

    /// <summary>
    ///  Read T_R022_1 data from XML document.
    /// </summary>
    /// <param name="elem">
    ///  Loaded XML document.
    /// </param>
    /// <returns>
    ///  parsed T_R022_1 data structure.
    /// </returns>
    static member private Read_T_R022_1 ( elem : XElement ) : T_R022_1 = 
        {
            D1 =
                Int32.Parse( elem.Element( XName.Get "D1" ).Value );
            R022_2 =
                ReaderWriter.Read_T_R022_2( elem.Element( XName.Get "R022_2" ) );
            D2 =
                Int32.Parse( elem.Element( XName.Get "D2" ).Value );
        }

    /// <summary>
    ///  Read T_R022_2 data from XML document.
    /// </summary>
    /// <param name="elem">
    ///  Loaded XML document.
    /// </param>
    /// <returns>
    ///  parsed T_R022_2 data structure.
    /// </returns>
    static member private Read_T_R022_2 ( elem : XElement ) : T_R022_2 = 
        {
            D1_1 =
                Int32.Parse( elem.Element( XName.Get "D1_1" ).Value );
            D1_2 =
                Int32.Parse( elem.Element( XName.Get "D1_2" ).Value );
        }
    ...
    static member private T_Test_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield! ReaderWriter.T_R022_1_toString ( indent + 1 ) indentStep ( elem.R022_1 ) "R022_1"
            yield sprintf "%s</%s>" indentStr elemName
        }

    /// <summary>
    ///  Write T_R022_1 data structure to configuration file.
    /// </summary>
    /// <param name="indent">
    ///  Indent space count.
    /// </param>
    ...
    ///  Array of the generated string.
    /// </returns>
    static member private T_R022_1_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_R022_1 ) ( elemName : string ) : seq<string> = 
        let indentStr = String.replicate ( indent * indentStep ) " "
        let singleIndent = String.replicate ( indentStep ) " "
        seq {
            yield sprintf "%s<%s>" indentStr elemName
            yield sprintf "%s%s<D1>%d</D1>" singleIndent indentStr (elem.D1)
            yield! ReaderWriter.T_R022_2_toString ( indent + 1 ) indentStep ( elem.R022_2 ) "R022_2"
            yield sprintf "%s%s<D2>%d</D2>" singleIndent indentStr (elem.D2)
            yield sprintf "%s</%s>" indentStr elemName
        }

    /// <summary>
    ///  Write T_R022_2 data structure to configuration file.
    /// </summary>
    /// <param name="indent">
    ///  Indent space count.
    /// </param>
    ...
    ///  Array of the generated string.
    /// </returns>
    static member private T_R022_2_toString ( indent : int32 ) ( indentStep : int32 ) ( elem : T_R022_2 ) ( elemName : string ) : seq<string> = 
        let indentStr = String.replicate ( indent * indentStep ) " "
        let singleIndent = String.replicate ( indentStep ) " "
        seq {
            yield sprintf "%s<%s>" indentStr elemName
            yield sprintf "%s%s<D1_1>%d</D1_1>" singleIndent indentStr (elem.D1_1)
            yield sprintf "%s%s<D1_2>%d</D1_2>" singleIndent indentStr (elem.D1_2)
            yield sprintf "%s</%s>" indentStr elemName
        }
