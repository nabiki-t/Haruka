...
// Definition of ConfRW_UT002_DateTime configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_DateTime
...
///  ConfRW_UT002_DateTime class imprements read and write function of configuration.
type ConfRW_UT002_DateTime() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_DateTime.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_DateTime.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                DateTime.SpecifyKind( DateTime( Int64.Parse( elem.Element( XName.Get "D1" ).Value ) ), DateTimeKind.Utc );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> DateTime.SpecifyKind( DateTime( Int64.Parse( itr.Value ) ), DateTimeKind.Utc ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( DateTime.SpecifyKind( DateTime( Int64.Parse( subElem.Value ) ), DateTimeKind.Utc ) );
            D4 = DateTime( 0L, DateTimeKind.Utc );
            D5 = DateTime.SpecifyKind( DateTime.Parse( "0001-02-03T04:05:06" ), DateTimeKind.Utc );
            D6 = DateTime.SpecifyKind( DateTime.Parse( "2025/01/01 0:00:00" ), DateTimeKind.Utc );
            D7 = 
                let subElem = elem.Element( XName.Get "D7" )
                if subElem = null then
                    DateTime.SpecifyKind( DateTime.Parse( "0002-03-04T05:06:07" ), DateTimeKind.Utc );
                else
                    DateTime.SpecifyKind( DateTime( Int64.Parse( subElem.Value ) ), DateTimeKind.Utc );
            D8 = 
                let subElem = elem.Element( XName.Get "D8" )
                if subElem = null then
                    DateTime.SpecifyKind( DateTime.Parse( "2025/01/01 0:00:00" ), DateTimeKind.Utc );
                else
                    DateTime.SpecifyKind( DateTime( Int64.Parse( subElem.Value ) ), DateTimeKind.Utc );
        }
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_DateTime.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_DateTime.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%d</D1>" singleIndent indentStr ( (elem.D1).Ticks )
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%d</D2>" singleIndent indentStr ( (itr).Ticks )
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%d</D3>" singleIndent indentStr ( (elem.D3.Value).Ticks )
            yield sprintf "%s%s<D7>%d</D7>" singleIndent indentStr ( (elem.D7).Ticks )
            yield sprintf "%s%s<D8>%d</D8>" singleIndent indentStr ( (elem.D8).Ticks )
            yield sprintf "%s</%s>" indentStr elemName
        }
