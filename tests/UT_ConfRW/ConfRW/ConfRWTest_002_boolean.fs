...
// Definition of ConfRW_UT002_boolean configuration reader/writer function.

namespace Haruka.Test.UT.ConfRW_002_boolean
...
///  ConfRW_UT002_boolean class imprements read and write function of configuration.
type ConfRW_UT002_boolean() =
...
<xsd:schema xmlns:xsd='http://www.w3.org/2001/XMLSchema'>
  <xsd:element name='Test' >
    ...
    static member LoadFile ( fname : string ) : T_Test =
        fname |> File.ReadAllText |> ConfRW_UT002_boolean.LoadString
    ...
    static member LoadString ( s : string ) : T_Test =
        ...
        "Test" |> xdoc.Element |> ConfRW_UT002_boolean.Read_T_Test
    ...
    static member private Read_T_Test ( elem : XElement ) : T_Test = 
        {
            D1 =
                Boolean.Parse( elem.Element( XName.Get "D1" ).Value );
            D2 =
                elem.Elements()
                |> Seq.filter ( fun itr -> itr.Name = XName.Get "D2" )
                |> Seq.map ( fun itr -> Boolean.Parse( itr.Value ) )
                |> Seq.toList
            D3 = 
                ...
                if subElem = null then
                    ...
                    Some( Boolean.Parse( subElem.Value ) );
            D4 = false;
            D5 = false;
            D6 = 
    ...
    static member WriteFile ( fname : string ) ( d : T_Test ) : unit =
        let s = ConfRW_UT002_boolean.T_Test_toString 0 2 d "Test"
    ...
    static member ToString ( d : T_Test ) : string =
        ConfRW_UT002_boolean.T_Test_toString 0 0 d "Test"
        |> String.Concat
    ...
    static member private T_Test_toString ( indent : int ) ( indentStep : int ) ( elem : T_Test ) ( elemName : string ) : seq<string> = 
        ...
        seq {
            ...
            yield sprintf "%s%s<D1>%b</D1>" singleIndent indentStr (elem.D1)
            if elem.D2.Length < 2 || elem.D2.Length > 3 then 
                raise <| ConfRWException( "Element count restriction error. D2" )
            for itr in elem.D2 do
                yield sprintf "%s%s<D2>%b</D2>" singleIndent indentStr (itr)
            if elem.D3.IsSome then
                yield sprintf "%s%s<D3>%b</D3>" singleIndent indentStr (elem.D3.Value)
            yield sprintf "%s%s<D6>%b</D6>" singleIndent indentStr (elem.D6)
            yield sprintf "%s</%s>" indentStr elemName
        }
